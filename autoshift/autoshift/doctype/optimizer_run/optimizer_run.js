...
// settled week is still the import's unconfirmed reading. A settled week nothing records
// yet needs no mention: the solve reads it straight off the Shift Schedule.
function confirm_and_solve(frm) {
	frappe.require("/assets/autoshift/js/rota.js", () => {
		Promise.all([
			...
			frm.call("check_unconfirmed_rotas"),
		]).then(([{ message: duplicates }, { message: binding }, { message: unconfirmed }]) => {
			...
			msg = autoshift.rota.unconfirmed_note(unconfirmed) + msg;

			frappe.confirm(msg, () => {
				frm.call("solve").then(() => frm.reload_doc());
			});
		});
	});
}
