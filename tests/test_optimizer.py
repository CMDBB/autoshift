...
def pkg(**overrides) -> DataPackage:
    ...
    The employee's designation appears in assistant_designations so active_rooms
    can be non-zero and the room-utilization objective can drive an assignment.
    ...
    base: dict[str, Any] = dict(
        ...
        rooms={(disc, b): 1},
        disciplines=[disc],
        assistant_designations={disc: ["Doctor"]},
        leave_blocked=set(),
        forced=set(),
        fte_tolerance=0.0,
        turnover_weight=1.0,
...
def test_discipline_without_assistants_has_zero_active_rooms():
    """
    If no employee's designation appears in assistant_designations for a discipline,
...
def test_max_rooms_per_employee_limits_cross_branch_assignment():
    ...
    total = (pulp.value(x[("E1", "AM", MON, "B1")]) or 0) + (
        pulp.value(x[("E1", "AM", MON, "B2")]) or 0
    )
    assert total <= 1 + 1e-6
...
def test_fairness_equalizes_unfairness_not_individual_balance():
    ...
    data = pkg(
        ...
        max_rpe={"E1": 1, "E2": 1},
        rooms={(disc, b): 2},
        assistant_designations={disc: ["Nurse"]},
        forced=forced_e1,
        fte_tolerance=0.0,
        turnover_weight=1.0,
    )
